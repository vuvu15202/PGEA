...
module.exports = function createRecord(req, res) {
  ...
  if (req.user && req.user.id) {
    // console.log('createdBy = ' + req.session.userId);
  ...
  async.reduce(_.keys(Model.attributes), [], function (memo, attrName, nextAttrName) {
    ...
    // if ((Model.attributes.id || { type: "" }).type === 'string') {
    //   data.id = uuidv1()
    // }
          ...
          if (req._sails.hooks.pubsub) {
            ...
            if (removedFromNotificationsToSend.length) {
              _.each(removedFromNotificationsToSend, function (notification) {
                Model._publishRemove(notification.id, notification.attribute, notification.removedId, !req.options.mirror && req, {
                  noReverse: true
                });
              });
            }
